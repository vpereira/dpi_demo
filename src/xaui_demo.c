...
//          printf("[%s] Starting jobs, numflows: %d, job: %p\n",__FUNCTION__, flag_numflows, job);
...
dataplane_thread (thread_info_t *info)
{
  ...
  printf("[%s] in, ring_id:%d\n", __FUNCTION__, master_config->ring_id);
  ...
  if (input_data_file)
    {
	  ...
	  printf("[%s] input_data_file : %s\n", __FUNCTION__, input_data_file);
  ...
  else if (flag_random_traffic)
    {
	  ...
	  printf("[%s] packet_memory is zeor\n", __FUNCTION__);
...
struct nlm_device_config *cli_config = NULL;

#if 0
static void
app_op_enable_cmd (uint32_t argc, char *argv[])
{
	if(cli_config == NULL){
		printf("[%s] config is NULL\n", __FUNCTION__);
		return;
	}
	print_regs(cli_config);
}

static void
app_normal_cmd(uint32_t argc, char *argv[])
{
	set_reg(cli_config, X0_CT_TEST, 0x00040000);
}

static void
app_lpbk_cmd(uint32_t argc, char *argv[])
{
	set_reg(cli_config, X0_CT_TEST, 0x00004000);
}

static void
app_init_cmd(uint32_t argc, char *argv[])
{
	init_regs(cli_config);
}

static void
app_test_cmd(uint32_t argc, char *argv[])
{
	unsigned int cmd;
	char *endptr, *str;
	...
	str = argv[1];
	cmd = (unsigned int)strtol(str, &endptr, 16);

	printf("X0_CT_TEST : 0x%x\n", cmd);
	set_reg(cli_config, X0_CT_TEST, cmd);

	return;

usage:
	printf("Usage : %s <val:0xXX> \n", __FUNCTION__);	
}

static void
app_cmd_cfg_cmd(uint32_t argc, char *argv[])
{
	unsigned int cmd;
	char *endptr, *str;
	...
	printf("coommand cfg : 0x%x\n", cmd);
	set_reg(cli_config, X0_COMMAND_CONFIG_, cmd);

	return;

usage:
	printf("Usage : %s <cmd:0xXX> \n", __FUNCTION__);	
}

#define MAXMESG 32
static cli_record_t app_dir_record;
static cli_record_t app_op_enable_record;
static cli_record_t app_normal;
static cli_record_t app_lpbk;
static cli_record_t app_init;
static cli_record_t app_test;
static cli_record_t app_cmd_cfg;
...
cli_run(void *arg)
{
	...
	char input[MAXMESG];
//	struct nlm_device_config *config = (struct nlm_device_config *)arg;

	cli_config = (struct nlm_device_config *)arg;

	strcpy(init_data.prefix, "root");
	cli_init(&init_data);

	rc = cli_mkdir("app", NULL, &app_dir_record);
	rc = cli_mkcmd("regs", app_op_enable_cmd, NULL, &app_op_enable_record);
	rc = cli_mkcmd("normal", app_normal_cmd, NULL, &app_normal);
	rc = cli_mkcmd("lpbk", app_lpbk_cmd, NULL, &app_lpbk);
	rc = cli_mkcmd("init", app_init_cmd, NULL, &app_init);
	rc = cli_mkcmd("test", app_test_cmd, NULL, &app_test);
	rc = cli_mkcmd("cfg", app_cmd_cfg_cmd, NULL, &app_cmd_cfg);
...
attach_ring (void *arg)
{
  ...
  printf("[%s] in, ring_id:%d, flag_binding:%d, flag_nthreads:%d\n", __FUNCTION__, ring_id, flag_binding, flag_nthreads);
...
main (int ac, char*av[])
{
 ...
 // ring_id = NLM_PCIE_RING_3;
  ...
  printf("rings:%d, flag_attach_mask:%d, ring_mask:%d\n", rings, flag_attach_mask, ring_mask);
